<template>
  <el-dialog v-model="dialogVisible" :title="title" :width="width" :destroy-on-close="destroyOnClose" :draggable="true" :close-on-click-modal="true" :close-on-press-escape="true">
    <!-- 添加一级目录按钮 -->
    <div class="mb-5">
  ...
  <!-- 添加文章到目录 -->
  <FormDialog ref="addArticle2CatalogDialogRef" title="添加文章" style="width: 80%" confirmText="添加" destroyOnClose @submit="onAddArticle2CatalogSubmit">
    <div>
      ...
      <el-card shadow="never">
        ...
        <el-table :data="tableData" border stripe v-loading="tableLoading" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="title" label="标题" width="380" align="center" />
          ...
          <el-table-column prop="isPublish" label="是否发布" width="100" align="center">
            <template #default="scope">
              <el-switch v-model="scope.row.isPublish" inline-prompt :active-icon="Check" :inactive-icon="Close" disabled />
            </template>
          </el-table-column>
...
import { showModel, showMessage } from '@/composales/utils'
